...
function PortalContent() {
  ...
  return (
    <div className="min-h-screen text-white font-sans bg-[#0A0F1C]">
      <nav className="border-b border-white/5 p-6 bg-[#11182B]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg overflow-hidden bg-blue-600">
              {shop?.logoUrl ? <img src={shop.logoUrl} alt={brandName} className="w-full h-full object-cover" /> : <Ticket size={20} />}
            </div>
            <div>
              <h1 className="text-xl font-black tracking-tighter uppercase">{brandName}</h1>
              {(shop?.address || shop?.phone) && <p className="text-xs text-slate-400 mt-1">{[shop?.address, shop?.phone].filter(Boolean).join(" · ")}</p>}
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto p-6 py-20 text-center">
        ...
        <p className="text-slate-300 text-lg mb-12 max-w-xl mx-auto">
          Ingresa el folio de tu orden para conocer el estatus actual de tu reparación en tiempo real.
        </p>
        ...
        <div className="relative max-w-2xl mx-auto">
          <input
            ...
            className="w-full bg-[#161B2C] border-2 border-blue-500/20 rounded-3xl px-8 py-6 text-xl outline-none transition-all text-center font-bold tracking-widest"
          />
          <button
            onClick={handleLookup}
            className="mt-8 w-full md:w-auto md:absolute md:right-3 md:top-3 md:mt-0 px-10 py-4 rounded-2xl font-black uppercase text-sm tracking-widest transition-all inline-flex items-center justify-center gap-2 bg-blue-600"
          >
            <Search size={16} />
            Consultar Estatus
          </button>
        ...
        {(orderLoading || error || order) && (
          <section className="mt-16 text-left bg-[#161B2C] border border-blue-500/10 rounded-[2rem] p-8 backdrop-blur-xl shadow-2xl shadow-blue-950/20">
            {orderLoading && (
            ...
            {!orderLoading && error && (
              <div className="rounded-3xl border border-amber-500/30 bg-amber-500/10 p-6 text-center">
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
                  <a href={whatsappHref} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-400 text-white font-black px-6 py-3 rounded-2xl transition-all">
              ...
              <div className="space-y-8">
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                  <div>
                    ...
                    <p className="text-slate-400 mt-2 max-w-2xl">{brandName} está trabajando en tu equipo.</p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0 lg:min-w-[320px]">
                    <div className="rounded-3xl border border-blue-500/10 bg-black/20 p-4">
                      ...
                      <p className="text-slate-400 text-sm">{[order.deviceBrand, order.deviceModel].filter(Boolean).join(" · ") || "Modelo no especificado"}</p>
                    </div>
                    <div className="rounded-3xl border border-blue-500/10 bg-black/20 p-4">
                ...
                <div className="rounded-3xl border border-blue-500/10 bg-black/20 p-6">
                      ...
                      return (
                        <div key={step} className={`rounded-3xl border p-4 transition-all ${active ? "border-blue-400/30 bg-blue-500/10" : "border-white/10 bg-white/5"}`}>
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className={`inline-flex h-8 w-8 items-center justify-center rounded-full text-xs font-black ${current ? "text-white bg-blue-600" : active ? "bg-blue-500/20 text-blue-200" : "bg-white/10 text-slate-400"}`}>
                              {index + 1}
                            </span>
                          </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-6">
                  <div className="rounded-3xl border border-blue-500/10 bg-black/20 p-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-white font-semibold">{order.resolution || "Tu técnico sigue trabajando en el diagnóstico y actualización del caso."}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-3xl border border-blue-500/10 bg-black/20 p-6">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <button onClick={() => window.print()} className="inline-flex items-center justify-center gap-2 border border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold px-6 py-3 rounded-2xl transition-all">
                        <Printer size={18} />
...
export function PortalBluePage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0A0F1C] flex items-center justify-center"><Loader2 className="animate-spin text-blue-500" size={40} /></div>}>
      <PortalContent />
    </Suspense>
  );
}
