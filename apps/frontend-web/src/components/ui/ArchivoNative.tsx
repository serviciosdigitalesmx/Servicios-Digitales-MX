    ...
    <section className="operativo-shell">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1rem" }}>
        <article className="sdmx-card-premium">
          ...
          <p className="muted" style={{ margin: "4px 0 0 0" }}>Órdenes cerradas y cobradas.</p>
        </article>
        <article className="sdmx-card-premium">
          ...
          <p className="muted" style={{ margin: "4px 0 0 0" }}>Casos que salieron sin conversión final.</p>
        </article>
        <article className="sdmx-card-premium">
          ...
          <p className="muted" style={{ margin: "4px 0 0 0" }}>Valor total visible del archivo cargado.</p>
        </article>
      </div>
        ...
        <article className="sdmx-card-premium" style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ borderBottom: "1px solid rgba(15,23,42,0.08)", paddingBottom: "16px", marginBottom: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div className="flex-col">
              ...
              <p className="muted" style={{ margin: 0, fontSize: "0.85rem" }}>Mostrando {archived.length} orden(es) en el historial.</p>
            </div>
          </div>
              ...
              archived.map((order) => (
                <li key={order.id} className="list-item-grid" style={{ background: order.status === "cancelado" ? "#fef2f2" : "#f8fafc" }}>
                  ...
                  <div className="flex-col">
                    ...
                    <span style={{ color: "#64748b", fontSize: "0.85rem" }}>Servicio: {order.deviceType} · Cierre: {formatDate(order.promisedDate)}</span>
                    <span style={{ color: "#94a3b8", fontSize: "0.75rem" }}>Prioridad original: {(order.priority || "normal").toUpperCase()}</span>
                  </div>
                  <div style={{ textAlign: "right", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" }}>
                    ...
                    <span style={{ fontSize: "0.95rem", fontWeight: "bold", color: normalizeArchiveStatus(order.status) === "cancelado" ? "#ef4444" : "#0f172a" }}>Costo final: {formatMoney(order.estimatedCost)}</span>
                  </div>
                </li>
              ))
            )}
