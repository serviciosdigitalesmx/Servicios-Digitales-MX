...
// ----------------------------------------------------------------------
// Helpers y utilidades (dentro del mismo archivo)
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Componentes internos (modulares pero dentro del mismo archivo)
// ----------------------------------------------------------------------

// 1. Header
function HubHeader({ session, onLogout }: { session: AuthMeResponse; onLogout: () => void }) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-bold text-white leading-none font-label uppercase tracking-wider">
              {session.shop.name}
            </p>
            <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-[0.2em] font-label font-bold">
...
function SubscriptionCard({ session, onUpgrade, upgrading }: { session: AuthMeResponse; onUpgrade: () => void; upgrading: boolean }) {
  ...
  return (
    <div className="sdmx-glass p-8 rounded-[2.5rem] flex flex-col justify-between relative overflow-hidden group min-h-[300px]">
      ...
      <div>
        ...
        <h3 className="text-3xl font-black text-white uppercase tracking-wider font-tech">{session.subscription.planCode}</h3>
...
// 3. Widget Portal Público
function PortalWidget({ session }: { session: AuthMeResponse }) {
  const [origin, setOrigin] = useState("");
  useEffect(() => {
    if (typeof window !== "undefined") setOrigin(window.location.origin);
  }, []);
  const portalUrl = `${origin}/portal?shop=${session.shop.slug}`;
...
// 4. Tarjeta de Acción (reutilizable)
function ActionCard({ href, title, description, icon, variant = "primary" }: {
  href: string;
  title: string;
  description: string;
  ...
  const baseClasses = "group sdmx-glass p-10 rounded-[2.5rem] transition-all block border-white/5 hover:border-blue-500/30";
  const iconContainerClasses =
    variant === "primary"
      ? "w-14 h-14 bg-gradient-to-br from-blue-600 to-blue-800 text-white rounded-2xl flex items-center justify-center mb-8 shadow-lg shadow-blue-500/20 group-hover:scale-110 transition-transform"
      : "w-14 h-14 bg-slate-800 text-slate-300 rounded-2xl flex items-center justify-center mb-8 border border-white/10 group-hover:bg-slate-700 transition-all";

  return (
    <a href={href} className={baseClasses}>
      <div className={iconContainerClasses}>{icon}</div>
      <h3 className="text-2xl font-black text-white uppercase tracking-wider font-tech">{title}</h3>
      <p className="text-slate-400 text-sm mt-3 font-medium leading-relaxed">{description}</p>
    </a>
  );
}

// 5. Grid de Acciones (basado en configuración)
const DEFAULT_ACTIONS = [
  {
    href: "/interno",
    title: "Sistema Central",
    description: "Control total de tu taller: Inventarios, Folios automáticos, Finanzas y control de stock en tiempo real.",
    icon: <IconDashboard width={28} height={28} />,
    variant: "primary" as const,
  },
  {
    href: "/billing",
    title: "Facturación y Plan",
    description: "Administra tus métodos de pago, historial de facturas y escala tu suscripción conforme crezca tu negocio.",
...
// ----------------------------------------------------------------------
// Componente principal del Dashboard (contenido interno)
// ----------------------------------------------------------------------
function HubDashboardContent() {
  ...
  return (
    <div className="sdmx-dashboard-container min-h-screen bg-slate-950">
      <HubHeader session={session} onLogout={handleLogout} />
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <SubscriptionCard session={session} onUpgrade={handleUpgrade} upgrading={upgrading} />
          <PortalWidget session={session} />
          <ActionsGrid />
        </div>
      </main>
    </div>
...
// ----------------------------------------------------------------------
// Exportación final (con AuthGuard)
// ----------------------------------------------------------------------
