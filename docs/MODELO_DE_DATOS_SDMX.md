...
- toda entidad operativa debe pertenecer a un `Shop`
- ninguna consulta debe mezclar datos entre tenants

Compatibilidad técnica actual:

- el esquema físico actual usa `tenant_id`
- ese `tenant_id` representa el `shop_id` del dominio
- mientras no hagamos una migración de renombre, ambos términos deben considerarse equivalentes
...
Representa el estado comercial de cada `Shop`.
...
- pertenece a un tenant / shop
- un shop puede tener una suscripción actual y conservar historial si luego se versiona el modelo

Reglas:

- debe existir índice por `tenant_id`
- debe existir índice por `status`
- el backend debe poder resolver rápido si un shop está habilitado o bloqueado
