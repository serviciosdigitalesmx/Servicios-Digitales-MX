...
Todo dato operativo debe poder asociarse al negocio dueño.

Regla oficial de dominio:

- el concepto de negocio principal se llamará `Shop`
- técnicamente, mientras la base actual siga usando `tenant_id`, se considera que `shop_id` y `tenant_id` representan la misma frontera de aislamiento
- toda evolución futura debe tender a un lenguaje de dominio centrado en `Shop`
...
### 4.5 Suscripción recurrente obligatoria

El sistema debe nacer listo para un modelo SaaS de cobro mensual.

Reglas:

- cada `Shop` debe tener una suscripción vigente o controlable
- la suscripción define si el acceso está `active`, `past_due`, `suspended`, `cancelled` o en prueba
- el backend debe poder bloquear operación cuando el estado comercial del `Shop` no permita acceso
- el precio inicial de referencia del producto es `350 MXN` mensuales

## 5. Módulos Oficiales del Producto

Además de los módulos funcionales, el producto debe incluir un dominio SaaS de plataforma para:

- gestión de shops
- suscripciones
- control de acceso comercial
- auditoría básica de estados de cuenta
...
- auth de usuarios
- control de shops / tenants
- roles y permisos
- generador de folios
- lógica de estados y semáforo
- CRUD de solicitudes, órdenes y catálogos
- validación de estado de suscripción por shop

## 6. Contrato de Multi-Tenancy

Todo dato del sistema nuevo debe modelarse con aislamiento por `Shop`.

Reglas:

- cada negocio es un `Shop`
- cada usuario pertenece a un `Shop`
- toda tabla operativa debe incluir referencia al `Shop`
- toda consulta del backend debe filtrar por `Shop`
- ningún endpoint puede devolver información cruzada entre shops

Compatibilidad técnica actual:

- mientras el esquema físico use `tenant_id`, ese campo será la implementación técnica de `shop_id`
- el lenguaje funcional y de negocio debe hablar de `Shop`, no de `tenant`, salvo en zonas de compatibilidad o migración

Tablas que obligatoriamente deben depender del shop:
...
- toda tabla operativa crítica debe poder resolverse por shop
- las suscripciones deben poder auditar monto, periodo y estado de cobro
...
- aislamiento estricto por `Shop`
- bloqueo de acceso por suscripción cuando aplique
